...
class GoogleApps
{
	...
	function Login($user, $domain, $pass)
	{
		...
		foreach (explode("\n", $result) as $line) {
			if (preg_match('#^Auth=(?P<token>.+)$#', $line, $matches)) {
	...
	public function ChangeUsername($domain, $from, $to)
	{
		$url = 'https://apps-apis.google.com/a/feeds/'.$domain.'/user/2.0/'.$from;
		...
		<atom:entry xmlns:atom="http://www.w3.org/2005/Atom"
		 ...
		 <apps:login userName="'.$to.'" />
		</atom:entry>';

		$result = $this->Request($url, 'PUT', $xml);
